...
int Pager(void) {
	...
	while (1) {
		...
		if (freeFrameFound != TRUE) {
			...
			while(freeFrameFound != TRUE){
				...
				for(p = 0; p < 1; p++){
					...
					if(processes[frames_list[p].process].pageTable[frames_list[p].page].clock == UNUSED && P1_GetPID() != frames_list[p].process){
						...
						P1_P(process_sem);
						processes[P1_GetPID()].pageTable[frames_list[p].page].state = INCORE;
						processes[P1_GetPID()].pageTable[frames_list[p].page].frame = freeFrameId;
			...
			DebugPrint("Before P #4\n");
						P1_P(process_sem);
						
							DebugPrint("here");
						result = USLOSS_MmuUnmap(0, frames_list[p].page);
						processes[P1_GetPID()].pageTable[frames_list[p].page].state = UNUSED;
						processes[P1_GetPID()].pageTable[frames_list[p].page].frame = -1;
		...
		if(processes[fault.pid].pageTable[page].block != -1){
			...
			DebugPrint("Before P #6\n");
			P1_P(process_sem);		
			P3_vmStats.pageIns++;
			
			DebugPrint("Before V #6\n");
			P1_V(process_sem);
			...
			processes[P1_GetPID()].pageTable[page].frame = freeFrameId;
			P1_V(process_sem);
			
			void *writeAddr = get_MMU_PageFrame_Address(page);
            ...
            result = USLOSS_MmuMap(0, page, freeFrameId, 3);
            if (result != USLOSS_MMU_OK) {
				USLOSS_Console("Pager(5): %d Mapping error, halting\n", result);
                USLOSS_Halt(1);
            }
			processes[P1_GetPID()].pageTable[page].state = INCORE;
			processes[P1_GetPID()].pageTable[page].frame = freeFrameId;
